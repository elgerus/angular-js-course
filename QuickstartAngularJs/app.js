...
bookApp.factory("kartService", function(){
  var kart = [];
  return {
    getKart: function(){
      return kart;
    ...
    buyBook: function(book){
      alert("Thanks for butying", book.name);
    }
  };
});

bookApp.factory("bookService", function(){
  var books = [{
		...
		},
		{
			...
			details: "Wings of Fire traces the life and times of former president A.P.J. Abdul Kalam. It gives a glimpse of his childhood as well as his growth as India's Missile Man. Summary of the Book Wings... View More"
    }];
  return {
    getBooks: function(){
      return books;
...
bookApp.controller("KartListCtrl", function($scope, kartService){
...
bookApp.controller("HeaderCtrl", function($scope, $location){
  $scope.appDetails ={
    title: "BookArt",
  ...
  $scope.nav.isActive = function(path){
    console.log("isActive " + $location.path);
...
bookApp.controller("BookListCtrl", function($scope, bookService, kartService){
  $scope.books = bookService.getBooks();
  $scope.addToKart = function(book){
      kartService.addToKart(book);
      console.log(`Add to Kart: ${book.name}`);
  }
});
